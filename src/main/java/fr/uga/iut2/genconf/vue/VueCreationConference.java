...
public class VueCreationConference extends javax.swing.JPanel {
    public static final IHM.InfosNouvelleConference VIDE = new IHM.InfosNouvelleConference(
                  "",
                  LocalDate.EPOCH,
                  LocalDate.EPOCH,
                  new IHM.InfosUtilisateur("", "", ""));
     ...
     */
    public VueCreationConference(IHMGenConf ihmGenConf) {
        this.ihmGenConf = ihmGenConf;
        this.nomsExistants = new HashSet<String>();
        this.valideAdmin = false;
        this.valideConf = false;

        initComponents();

        // Format de date (à faire après création des composants
        JSpinner.DateEditor editor = new JSpinner.DateEditor(dateDebut, "dd/MM/yyyy");
        dateDebut.setEditor(editor);

        creer.setEnabled(false);
    }

    public void setNomsExistants(final Set<String> nomsExistants) {
        if (nomsExistants != null) {
    ...
    private void initComponents() {
        ...
        jPanel2.setBorder(javax.swing.BorderFactory.createTitledBorder("Administrateur"));
    ...
    private void creerActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_creerActionPerformed
         ...
         Date debut = (Date) dateDebut.getModel().getValue();
         ...
         IHM.InfosNouvelleConference nlleConf = new IHM.InfosNouvelleConference(
                 nomConference.getText(),
                 dateDebut,
                 dateFin,
                 admin);

         ihmGenConf.creerConference(nlleConf);
    ...
    private void verificationAdmin(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_verificationAdmin
        ...
        nom = adminNom.getText();
        prenom = adminPrenom.getText();
        validEmail = validator.isValid(adminEmail.getText());
        adminEmail.setForeground(validEmail ? Color.black : Color.red);

        valideAdmin = validEmail
                 && (nom != null) && (nom.length() > 0)
                 && (prenom != null) && (prenom.length() > 0);

        creer.setEnabled(valideAdmin && valideConf);
    ...
    private void verificationConference(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_verificationConference
         valideConf = !nomsExistants.contains(nomConference.getText());
         nomConference.setForeground(valideConf ? Color.black : Color.red);
         creer.setEnabled(valideAdmin && valideConf);
    }//GEN-LAST:event_verificationConference

    private void annulerActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_annulerActionPerformed
        ihmGenConf.creerConference(VIDE);
